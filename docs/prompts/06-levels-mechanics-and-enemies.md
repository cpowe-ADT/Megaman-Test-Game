...
- **6.0 adds the hit contract (EVAL-P6-015).** One `resolveHurtbox(entity)` read by the sword, shots and the debug overlay (today the sword uses the drawn frame, `Game.ts:1960`, shots the body, `EnemyMotor.ts:88`, and the catalog hurtbox is never read); enemy melee tests against the player body profile, not `getBounds()` (`EnemyCombat.ts:163`), with `tests/enemy-melee-hitbox.test.ts`; (the reviewer's "colliders added four times" was checked and is not a bug: `attachActor` clears the actor's colliders first, `PlatformCollisionSystem.ts:115`; keep that with a test when the wiring moves); dead `applySaberDamage` deleted; the overlap wiring moves to `src/scenes/game/HitWires.ts` here, not in 7.0. EVAL-P6-014's target drops to 2,700 lines.
...
## Phase 6.P: The tutorial pilot, to a game standard (D-016, 2026-09-24)

Craig after playing 05c: "horrible design ... not to the standard of a game", the dash screen's spike strip read as "a boss fight which is impossible to play". The tutorial is the first thing a player sees, so it is rebuilt first, in three lanes that touch different files, and the pattern then repeats per stage in 6.4 to 6.7.

- **Engine lane (EVAL-P6-017).** Platforms, ground, walls and spikes drawn from a per-biome 16px tileset atlas (`tiles_<biome>`, contract in `src/stage/tileSkin.ts`, pure frame-picking tested) with the rectangles as the fallback; collisions unchanged. The HUD says nothing about a boss outside the boss room: the boss panel, bar and name appear at the fight, the centre label shows the district until then.
- **Art lane (EVAL-P6-018).** Through Higgsfield `gpt_image_2`: the relay biome tileset (ground top and fill, ledge caps and fill, one-way plank, wall faces, spike strip, decor) and a three-layer parallax background (far skyline, mid hangar structure, near girders), on the style sheet's relay palette; cut to 16px tiles and 448px tileable layers; provenance and attribution before runtime use.
- **Level lane (EVAL-P6-019).** The tutorial reads as a place and teaches without killing: the dash screen teaches the dash on a safe floor (a gap you can fall into and walk back from, no spikes on the first dash teach); spikes appear only after the player has used the verb they test; the 05b carry-overs (enemy roster, the secret behind the saber wall, a crumble group) land here; smoke 49's replays updated with the geometry.
- **STOP 6.P:** Craig plays the tutorial. Show: before and after captures of every screen at 2x, the tileset and background sheets. Question: is this the standard to repeat on every stage? Recommended: yes, then Pyro Maw next.
