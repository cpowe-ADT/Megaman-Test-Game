...
| EVAL-P6-010 | gate + review | nine non-Omega stages pass audit and lint | PENDING | stage registry (804c87e); Water District rebuilt: 12 screens, currents (jump 80%), the water-level gate, 19 placements, both secrets (5ab3963, b0290b2, 6899cb2; `tests/tide-reaver-stage.test.ts`, smoke `55-tide-route`); left: Mire, Volt, Basalt (running), Ferro, Gale, Glacier, and Craig's play |  |
...
| Id | Kind | What passes | Status | Evidence | Commit |
| --- | --- | --- | --- | --- | --- |
| EVAL-P7-001 | gate | twelve hazard spawners; telegraphs drawn; watchdog gone | PASS | a spawner with its own body, art and timing per hazard id (icicle_fall, charge_mine, tornado_pillar, splash_pillar, burn_puddle, acid_trail, magnet_node, stone_pillar, vapor_pod, short_quake, ground_shockwave, wind_hitbox, mag_disc) and the Pyro and Tide pilots, `hazards_v1` art (3e35aa9, 7547baf); `git grep Watchdog src` and `git grep dash_strike src` print nothing; telegraphs since a02ac2c; `tests/boss-hazard-spawners.test.ts` (16), smoke 53 one hazard per warden (`output/evidence/12-wave4/stop-12f/pyro-tide-fights.png`); `npm run -s gate -- verify` on 9dde2e9 -> `PASS verify (448s)`, `# pass 650`, `Smoke test complete: 62 ran`, 0 failed (`output/smoke-runs/2026-09-25T07-48-20-983Z`); production-build smoke 8 ran, 0 failed; sweep pass; `footprint: 22/22` (`output/evidence/12-wave4/`) | 9dde2e9 |
| EVAL-P7-002 | gate | phase kits, desperation, weakness stagger on Pyro and Tide | PASS | all nine wardens and the Core (0e90883): phase two retires, retimes and adds one attack, desperation at 20% adds one, weakness >=1.4 stuns 200 ms; `tests/boss-phase-kits.test.ts` and smoke `44-boss-beats` (Pyro, Tide); `npm run -s gate -- verify` -> `PASS verify (402s)`, `Smoke test complete: 59 ran`, 0 failed (`output/smoke-runs/2026-09-25T05-33-58-145Z`); sweep pass (`output/sweep-runs/2026-09-25T05-39-48-188Z`); `footprint: 22/22` on 6233a5e (`output/evidence/12-wave2/`); the weakness break (Craig's ask, 2026-09-25): any attack phase cancels, 24 px knockback with a hop, the recoil pose, 450 ms stun held until landing, 1.5 s lockout (14cc6c0, 0152138; `tests/boss-break.test.ts`, smoke 44) | 6ac7a59 |
...
| EVAL-P7-004 | gate | weapon identities; weakness ring; no BLOCKED in Classic | PASS | eight weapon identities with their `weapons_v1` art and on-hit tags, the weakness table (1.75, 1, 0.75 `RESISTED HIT`; no BLOCKED in Classic; Rook takes the Buster only; Omega rotates Lightning, Metal, Ice), HUD weapon icons (984fdd7, 861abfc, 3538eab); `tests/weapon-identities.test.ts` (24), smoke 12 one shot per weapon (`output/evidence/12-wave3/weapons-sheet.png`); `npm run -s gate -- verify` on 08be26e -> `PASS verify (392s)`, `# pass 615`, `Smoke test complete: 60 ran`, 0 failed (`output/smoke-runs/2026-09-25T06-20-07-817Z`); sweep pass (`output/sweep-runs/2026-09-25T06-26-37-193Z`); `footprint: 22/22 within budget` (`output/evidence/12-wave3/`) | 08be26e |
...
| EVAL-P7-010 | gate | bosses split into floor body, hurtbox and per-phase attack hitboxes (roster.ts, BossDamageRouter); projectiles hit platforms by body | PASS | every boss has a floor body, a hurtbox and a hitbox per attack phase in `src/bosses/roster.ts`, routed by `BossDamageRouter`; idle contact 2 (7547baf); shots hit platforms by body, a level shot grazing the floor flies on (315d219); `tests/boss-bodies.test.ts` (6), `tests/projectile-platform-contact.test.ts`, smoke 53 (the saber lands on the hurtbox, the floor body is harmless, a jumping hero clears the shockwave); `npm run -s gate -- verify` on 9dde2e9 -> `PASS verify (448s)`, `# pass 650`, `Smoke test complete: 62 ran`, 0 failed (`output/smoke-runs/2026-09-25T07-48-20-983Z`); production-build smoke 8 ran, 0 failed; sweep pass; `footprint: 22/22` (`output/evidence/12-wave4/`) | 9dde2e9 |

## Prompt 08: Audio, presentation, and release

| Id | Kind | What passes | Status | Evidence | Commit |
| --- | --- | --- | --- | --- | --- |
| EVAL-P8-001 | gate | cue map test; per-screen music | PASS | `tests/audio-cue-map.test.ts` green; Game and BossBeats pass stage and boss ids, and phase two switches to the phase-two layer (4b6ece2); automation shows `bgm_stage_<id>` per stage and `bgm_boss_<id>` then `_phase2` per boss (`output/evidence/12-wave3/music-cues-report.json`); `npm run -s gate -- verify` on 08be26e -> `PASS verify (392s)`, `# pass 615`, `Smoke test complete: 60 ran`, 0 failed (`output/smoke-runs/2026-09-25T06-20-07-817Z`); sweep pass (`output/sweep-runs/2026-09-25T06-26-37-193Z`); `footprint: 22/22 within budget` (`output/evidence/12-wave3/`); the pickups commit 6db94e0 also carries this id by a task-card slip | 08be26e |
...
| EVAL-P8-005 | gate | gamepad, remap, options; smoke 46 | PASS | a pad source on the one action map (deadzone 0.25, the default map), the remap screen (keyboard and pad columns, press-to-bind, conflicts, reset; `settings.v1` with a migration test), Options: Fullscreen, Pixel scaling Smooth or Integer, Reduced flashing, hidden-tab pause; hazard patterns checked (69a6c6e, 68326d7); `tests/pad-remap-options.test.ts`, smoke 46 (`output/evidence/12-wave4/shot-remap-screen.png`, `shot-options-integer.png`); touch finished per D-020: weapon buttons, menus by tap, TOUCH CONTROLS AUTO / ON / OFF, smoke 4c taps (17a83ba, 86ccdec); `npm run -s gate -- verify` on 9dde2e9 -> `PASS verify (448s)`, `# pass 650`, `Smoke test complete: 62 ran`, 0 failed (`output/smoke-runs/2026-09-25T07-48-20-983Z`); production-build smoke 8 ran, 0 failed; sweep pass; `footprint: 22/22` (`output/evidence/12-wave4/`) | 9dde2e9 |
...
| Id | Kind | What passes | Status | Evidence | Commit |
| --- | --- | --- | --- | --- | --- |
| EVAL-P12-001 | gate | CI runs smoke and sweep on every pull request, not only by hand | PASS | `.github/workflows/ci.yml` runs smoke and the sweep on pull requests to main (e9781ef); PR #59 run 36110503227 (pull_request, 86f8d76): test-and-build success, browser-gates success on Ubuntu -> `Smoke test complete: 62 ran`, `Mission visual sweep complete` (`output/evidence/12a/ci-run-36110503227.txt`); the Linux-only failures before it were font metrics (fb5e1c1, d7efc20) and a slow runner's freeze read (9dde2e9) | 86f8d76 |
...
| EVAL-P12-004 | gate | a sound for every new action (combo hits, air spin, reflect, vents, slag, walls, crumbles, gates, mini-bosses); unknown SFX keys throw in development | PASS | combo hits, air spin, reflect, mini-boss stomp and shockwave, WARNING wired (4b6ece2), mechanics sounds earlier (1d965ee..9c58106); unknown keys throw in development; `tests/saber-sounds.test.ts`, `tests/audio-sfx-keys.test.ts`, `tests/audio-cue-map.test.ts` (desperation attacks included since 6ac7a59); `npm run -s gate -- verify` on 08be26e -> `PASS verify (392s)`, `# pass 615`, `Smoke test complete: 60 ran`, 0 failed (`output/smoke-runs/2026-09-25T06-20-07-817Z`); sweep pass (`output/sweep-runs/2026-09-25T06-26-37-193Z`); `footprint: 22/22 within budget` (`output/evidence/12-wave3/`) | 08be26e |
| EVAL-P12-005 | audit | the JS budget holds the finish features: `jsGzipKB` 510 -> 520 (a raise needs this row) | PASS | wave 4 (boss bodies, twelve spawners, pad and remap) measured 514.5 KB after the dialogue left the bundle (e71e3ec, -10 KB); the enemy catalog and manifest notes followed (-2.2 KB), leaving 512.3 KB; the rest is new code, so 520 (+2%) until part 12d loads stage data per stage, then the ceiling comes back down; `footprint: 22/22 within budget, 0 page errors` on 9dde2e9, `static.jsGzipKB 512.6 / 520` (`output/evidence/12-wave4/footprint-9dde2e9.json`) | 9dde2e9 |
