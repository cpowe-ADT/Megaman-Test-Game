...
| EVAL-P10-006 | gate | (10b) CI runs `npm run agents:check` on every push | PENDING (`D-011`) | | |
| EVAL-P10-007 | gate | (10b) evidence tied to commits: summaries carry `commit` and `dirty`; per-eval snapshots | PENDING | | |
| EVAL-P10-008 | gate | (10b) a test builds every part's pack under 30K; 06 and 08 carry the sections they cite from 02 and 04 | PENDING | | |
| EVAL-P10-009 | review | (10b) retro at each prompt exit proposes at most one rule change as a DECISIONS row | PENDING | | |
