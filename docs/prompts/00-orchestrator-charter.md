...
| Input | `src/input/InputActions.ts` handles Enter, Space and Esc only; `Game.ts:2279` binds the gameplay keys itself. The touch pad has eight buttons and no weapon switch. No gamepad. | `src/input/`, `src/ui/GameplayTouchControls.ts` | 01 (action map), 04 (pad, remap) |
...
| --- | --- | --- |
| `npm run test` | 185 scene/system/content/tooling tests plus 12 boss-framework tests (180 + 12 at prompt 01 entry) | green, run 2026-09-10; `output/phase-1-0/09-final-test.log` |
| `npm run build` | typecheck, bundle, `dist/assets` completeness (153 files today, of which 23 are private; a public build has 130) | green, run 2026-09-10; `output/phase-1-0/10-final-build.log` |
| `npm run test:smoke` | 38 Playwright scenarios (highest numbered id is 32), writes `output/web-game-smoke/summary.json` | The 2026-08-05 failure at `29-pellet-hits-short-enemy` did not reproduce at prompt 01 entry on 2026-09-10: 38/38 passed before any runtime edit. Preserved evidence: `output/phase-1-0/baseline-smoke/summary.json`; scenario 29's state shows a live mine bot at 4 HP after one uncharged Buster hit. Prompt 01 strengthens its former false-positive predicate. |
