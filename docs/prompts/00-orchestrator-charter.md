...
| Debt | `src/scenes/Game.ts` is 3,805 lines during Phase 1.1 (3,846 after 1.0b; 3,928 at entry) and the only `@ts-nocheck` file; its debug-hook block is initialized after the action adapter and resets on shutdown for reentry. The Phaser chunk is about 1.45MB; ADR-0002 accepts the warning. | | every prompt shrinks it |
...
| --- | --- | --- |
| `npm run test` | 218 scene/system/content/tooling tests plus 12 boss-framework tests (180 + 12 at prompt 01 entry) | green, run 2026-09-10; `output/phase-1-1/21-verify.log` test component |
| `npm run build` | typecheck, bundle, `dist/assets` completeness (153 files today, of which 23 are private; a public build has 130) | green, run 2026-09-10; `output/phase-1-1/21-verify.log` build component |
| `npm run test:smoke` | 41 Playwright scenarios including Classic 33/33b, writes `output/web-game-smoke/summary.json` | 41/41 on 2026-09-10; `output/phase-1-1/25-full-smoke-final.log`, preserved `full-smoke/summary.json`. First verify failed 29; traced target mine interception required fixture isolation, not a sensor change. Strict live HP/contact assertions remain. |
| `npm run test:visual-sweep` | 10 missions x start/mid/pre-boss/boss-room, writes `output/mission-visual-sweep/summary.json` | 10/10 on 2026-09-10; `output/phase-1-1/27-visual-sweep.log`, preserved `visual-sweep/summary.json`; screenshots retain documented presentation debt |
| `npm run sprites:validate` | manifest and coverage | green; `output/phase-1-1/21-verify.log` sprite component |
| `npm run verify` | validate + test + build + smoke (the sweep is separate) | First invocation failed smoke 29 (`21-verify.log`); unchanged sprites/test/build components passed. After fixture isolation, full smoke 41/41 (`25-full-smoke-final.log`) completes all components. |
