...
export function createPauseOverlay(scene: Phaser.Scene, width: number, height: number): Phaser.GameObjects.Container {
  ...
  const label = scene.add.text(width / 2, height / 2, 'Paused', { fontFamily: 'monospace', fontSize: '22px', color: '#ffffff', backgroundColor: 'rgba(8, 12, 20, 0.75)', padding: { x: 12, y: 8 }, align: 'center' }).setOrigin(.5).setScrollFactor(0).setShadow(2, 2, '#000000', 4, true, true)
