...
- Review seats are project subagents in `.claude/agents/game-*.md`. Launch the ones a slice needs in a single message so they run in parallel and blind; each reads only its seat file in `docs/prompts/seats/`. The user-level agents on this machine (`coder`, `measurement`, `documentation` and the report seats) belong to another project: do not use them here.
